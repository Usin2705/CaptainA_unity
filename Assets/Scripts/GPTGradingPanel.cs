...
{
    [SerializeField]
    GameObject gptGradingPanel;

    [SerializeField]
    TMPro.TextMeshProUGUI transcriptText;

    [SerializeField]
    TMPro.TextMeshProUGUI gptGradingText;
        ...
        IEnumerator LoadTTSClipCoroutine()
        {
            ...
            if (ttsClip != null)
            {
                ...
                ttsButton.onClick.AddListener(() =>
                    AudioManager.GetManager().PlayAudioClip(ttsClip)
                );
            }
        }
    }
    ...
    */
    {
        if (!String.IsNullOrEmpty(filename))
        {
            ...
            using (
                var uwr = UnityWebRequestMultimedia.GetAudioClip("file://" + path, AudioType.MPEG)
            )
            {
